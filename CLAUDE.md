...
| `docs/OBJECT_EVAL.md` | **Genauigkeits-Eval** der Objekt-Detektion (mAP/IoU) + 3D-Lokalisierung (Position/Yaw/Größe) gegen UAVScenes-Masken + LiDAR-Label-Referenz; Methodik + alle Ergebnisse (OD-Modell-Vergleich, Frame-Selector-Entkopplung, Stride-Sweep). Tools `scripts/eval/{object_eval,detect_on_frames,refuse_stride}.py`; Kernzahl **~3 m Position / ~15° Yaw**. Konsolidiert in `output/exp_interval1_objects_eval/EVAL_SUMMARY.md` |
| `docs/STREAMING.md` | Live-Streaming von Gaussians (`.splat`) + Objekt-Markern zu einem Web-Frontend während des Runs via WebSocket: `stream.enabled: true`; frozen/active-Delta (StorageManager append-only vs Mapper full-replace, Key `_globalkf_id`); non-blocking daemon-Thread; `scripts/server/{stream_server,splat_encode}.py` + three.js-Frontend `static/viewer.html` (3D-Karte, Disks/Splats-Toggle, OD-Marker; `test_viewer.html` = Smoketest); braucht `pip install websockets`. Objekte streamen mit Orientierung+Größe (`quat`/`size` via PCA in `object_tracker.estimate_pose_size`) und werden als **echte glTF-Modelle pro Klasse** (`static/models/`, `MODEL_REGISTRY`, Box-Fallback, models-Toggle) gerendert; Detektion läuft auf jedem N-ten Tracker-KF (`object_detect_stride`, Default 3, vom FrameSelector entkoppelt) |
