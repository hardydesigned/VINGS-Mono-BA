...
BASE=configs/local/interval1/interval1_lidar_full.yaml
CHUNKDIR=output/exp_interval1_lidarchunks
mkdir -p "$CHUNKDIR"
PLYS=()
...
s = re.sub(r"output:\s*\{save_dir:[^}]*\}",
           f"output: {{save_dir: /home/philipp/Dokumente/Github/VINGS-Mono-BA/output/exp_interval1_lidarchunks/c{start}/}}", s)
open(cfg, "w").write(s)
PYEOF
  echo "=========== LIDAR-CHUNK start=$START len=$CHUNK ==========="
  $PY scripts/run_experiment.py "$cfg" > "$CHUNKDIR/run_$START.log" 2>&1
  d=$(ls -dt output/exp_interval1_lidarchunks/c$START/*/ 2>/dev/null | head -1)
...
$PY scripts/eval/merge_plys.py --out "$CHUNKDIR/survey_lidar_complete.ply" --opacity-min 0.2 "${PLYS[@]}"
echo "=========== FERTIG -> $CHUNKDIR/survey_lidar_complete.ply ==========="
