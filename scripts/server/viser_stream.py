...
class ViserStreamServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 8765,
                 ...
                 render_mode: str = "gaussians", **_ignored):
        if viser is None:
            raise RuntimeError(
        ...
        with self._lock:
            ...
            if not ids:
                ...
                if mode == "gaussians":
                    ...
                    # last-line guard: ONE non-finite value crashes the WASM splat
                    # sorter and blanks the whole scene -> drop any that slipped in.
                    good = (np.isfinite(xyz).all(1)
                            & np.isfinite(cov.reshape(cov.shape[0], -1)).all(1)
                    ...
                    if self._gh is None:
                        ...
                        with self.server.atomic():
                            self._gh.centers = xyz; self._gh.covariances = cov
                            self._gh.rgbs = rgb; self._gh.opacities = opac
                    ...
                    if self._ph is None:
                        ...
                        with self.server.atomic():
                            self._ph.points = xyz; self._ph.colors = rgb
                            self._ph.point_size = self._point_size()
                            self._ph.point_shape = self._point_shape
    ...
    def push(self, payload: dict):
        ...
        try:
            ...
            if t == "kf":
                ...
                self._clear_scene()
            # 'detections' (camera PiP) intentionally not mirrored into the 3D
            # scene; the run still writes the overlay video.
        except Exception as e:
