#!/usr/bin/env python3
"""Generate the s3100_200f sweep configs for amtown03 (frames 3100..3299).

Uses configs/local/amtown03/s3100_200f/amtown03_short_200_template.yaml as the
base. One YAML per variant — grouped into subfolders the run_sweep.sh
`--s3100` mode iterates over.
...
Output goes to configs/local/amtown03/s3100_200f/<group>/amtown03_s3100_200f_<variant>.yaml
"""
...
TARGET_ROOT = REPO / "configs/local/amtown03/s3100_200f"
SAVE_DIR = str(REPO / "output/exp_amtown03_s3100_200f") + "/"

NAME_PREFIX = "amtown03_s3100_200f"
...
def main() -> None:
    ...
    print(f"\n=== amtown03 s3100_200f — {len(plan)} configs ===")
    for group, variant, cfg in plan:
        path = TARGET_ROOT / group / f"{NAME_PREFIX}_{variant}.yaml"
        header = (
            f"# Auto-generated by scripts/gen_s3100_200f_configs.py — DO NOT EDIT.\n"
            f"# dataset = amtown03 (frames 3100..3299)\n"
            f"# group   = {group}\n"
            f"# variant = {variant}\n"
        )
